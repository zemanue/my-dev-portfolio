...
function SkillCard({ skill }) {
    return (
        <div
            key={skill.name}
            className="relative flex flex-col items-center p-4 rounded-xl bg-zinc-100 dark:bg-zinc-900 text-zinc-800 dark:text-zinc-100 shadow transition-all duration-300 border border-zinc-200 dark:border-zinc-700 hover:scale-105 hover:shadow-xl group cursor-pointer"
        >
            ...
            {/* Tooltip */}
            {skill.description && (
                <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-48 p-2 bg-zinc-800 dark:bg-zinc-700 text-white text-xs rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 pointer-events-none z-10">
                    {skill.description}
